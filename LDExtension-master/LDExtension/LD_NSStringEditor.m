...
- (BOOL)isOnlyWhitespaceAndNewlineCharacter {
    if (self.length>0 && [[self trimmingWhitespaceAndNewlineCharacter] isEqualToString:@""]) {
        return YES;
    }else {
        return NO;
    }
...
- (BOOL)includeBothNumberAndLetter {
    if ([self isOnlyNumber] || [self isOnlyLetter] || ![self isOnlyNumberAndLetter]) {
        return NO;
    }else {
        return YES;
    }
...
-(NSString *)clipWithMaxCharacterlength:(int)length {
    NSString *tmp = self;
      while ([tmp LD_characterLength] > length) {
